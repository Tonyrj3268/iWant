<?php 
    ...
    $item=$_POST['item'];
    $query = "SELECT stuff_ID,stuff_status,stuff_topic,stuff_content,stuff_img_name,stuff_price,stuff_place,user_account FROM stuff_info where stuff_ID ='$item';";
                ...
                if (mysqli_num_rows($result)>0) {
                    while($row = $result->fetch_assoc()) {
                        if($row["stuff_img_name"]){
                            echo '
                                <input type="hidden" name="id" value='.$row["stuff_ID"] .'></input>
                                <a>Status: '.$row["stuff_status"] .'</a><input type="hidden" name="status" value='.$row["stuff_status"] .'></input><br>
                                ...
                                <a id="user_name" name="user" onclick="chat('.$row["user_account"] .')">From: '.$row["user_account"] .'</a><input type="hidden" name="user" value='.$row["user_account"] .'></input><br>
