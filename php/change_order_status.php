    ...
    if($who_change=='seller'){
        ...
        if (mysqli_num_rows($result)>0) {
            while($row = $result->fetch_assoc()) {
                $status=$row['seller_status']-1;
                $query2 = "update stuff_record set seller_status=".$status." where record_id=$record_id";
                $result2 = mysqli_query($conn,$query2);
                
                if(send_notify_to_buyer($record_id,$row['borrower_id'],$row['seller_status'],$conn)){
        ...
        if ($result){
            if (mysqli_num_rows($result)>0) {
                while($row = $result->fetch_assoc()) {
                    $status=$row['buyer_status']-1;
                    $query2 = "update stuff_record set buyer_status=".$status." where record_id=$record_id";
                    $result2 = mysqli_query($conn,$query2);
                    
                    if(send_notify_to_seller($record_id,$row['borrower_id'],$row['buyer_status'],$conn)){
    ...
    function send_notify_to_buyer($record_id,$to_user_id,$status,$conn){
        ...
        switch($status){
            case 4:
                ...
                break;
            case 3:
                $topic='賣家('.find_seller_ID($record_id,$conn).')已收回商品(訂單編號:'.$record_id.')';
                $content='賣家('.find_seller_ID($record_id,$conn).')已收回商品(訂單編號:'.$record_id.')，感謝您使用本服務，歡迎您至<個人頁面><購物車>為商品進行評價!';  
                break;  
        }
        $query = "
